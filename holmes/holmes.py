...
def makeChemProto(name='hydra'):
        ...
        #Adot.expr="x6*(-x0+(5/(1+x1^3))*(2-x0-x3))"
        #Bdot.expr="x6*(0*x0-x1+(3+2*(x2^3/(1+x1^3)))*(1/(1+x0^3))*(2-x1-x4))"
        #Cdot.expr="x6*(0*x0+0*x1-0*x2+0.1*((0.2+0.3*(x0^3/(0.85^3+x0^3)))-x2^2*(0.2+1*x1^3/(0.85^3+x1^3))))"
        Adot.expr="x6*(-x0+(5/(1+x1^3))*(2-x0-x3))"
        Bdot.expr="x6*(0*x0-x1+(3+2*(x2^3/(1+x1^3)))*(1/(1+x0^3))*(2-x1-x4))"
        Cdot.expr="x6*(0*x0+0*x1-0*x2+0.1*((0.2+0.3*(x0^3/(0.85^3+x0^3)))-x2^2*(0.2+1*x1^3/(0.85^3+x1^3))))"
        

        Ddot.expr="x6*(0*x0+0*x1+0*x2-x3+(5/(1+x4^3))*(2-x0-x3))"
        Edot.expr="x6*(0*x0+0*x1+0*x2+0*x3-x4+(3+2*(x5^3/(1+x4^3)))*(1/(1+x3^3))*(2-x1-x4))"
        Fdot.expr="x6*(0*x0+0*x1+0*x2+0*x3+0*x4-0*x5+0.1*((0.2+0.3*(x3^3/(0.85^3+x3^3)))-x5^2*(0.2+1*x4^3/(0.85^3+x4^3))))"
        ...
        moose.connect( Z, 'nOut', Adot.x[6], 'input' )
        ...
        moose.connect( Z, 'nOut', Bdot.x[6], 'input' )
        ...
        moose.connect( Z, 'nOut', Cdot.x[6], 'input' )
        ...
        moose.connect( Z, 'nOut', Ddot.x[6], 'input' )
        ...
        moose.connect( Z, 'nOut', Edot.x[6], 'input' )
        ...
        moose.connect( Z, 'nOut', Fdot.x[6], 'input' )
...
def main():
    ...
    Z = moose.element('/model/chem/dend/Z')
    ...
    moose.element('/model/chem/dend/Z').vec.nInit = 1
    #moose.element('/model/chem/dend/Z').vec[50].nInit = 1
    
    moose.element('/model/chem/dend/A').vec[50].nInit=0.1
    moose.element('/model/chem/dend/B').vec[50].nInit=0.1
    moose.element('/model/chem/dend/C').vec[50].nInit=0.1
    moose.element('/model/chem/dend/D').vec[10].nInit=0.2
    moose.element('/model/chem/dend/E').vec[10].nInit=0.2
    moose.element('/model/chem/dend/F').vec[10].nInit=0.2
    storeAvec=[]
    storeBvec=[]
    storeCvec=[]
    storeDvec=[]
